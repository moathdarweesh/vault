...
Then grep for the old number — zero matches should remain. **Current version: v128.** (The `app.js` build label and the feedback version tag both read the single `VAULT_BUILD` const at the top of `app.js` — bump that one const, not two literals.)
