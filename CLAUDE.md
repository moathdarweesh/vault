...
**Current version: v255.** APK: build 16 / v2.5.
...
- **Error visibility.** `Cloud.reportError()` + `window.onerror`/`unhandledrejection` write to `client_errors` (`backend/pending/client-errors-v9.sql`): signed-in users only, no user content, per-session dedupe, DB-side rate cap of 20/hour, 30-day retention via `admin_prune_client_errors()`. The reporter must never throw and never block.
...
- **Pending SQL (written, not yet applied):** `backend/pending/client-errors-v9.sql`, `backend/pending/hardening-v8.sql` (revoke PUBLIC execute + pin `search_path`), `backend/pending/ban-rls-v10.sql` (extend the ban to the mirror tables, storage and profiles). (`launch-hardening.sql` too.) The earlier audit's `09_hardening-v5.sql` is NOT pending — it was applied in `e54cfed`; this line said otherwise for three weeks. **`backend/README.md` is now the authority for what is applied**, derived from git rather than memory.
