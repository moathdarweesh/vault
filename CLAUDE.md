...
**Current version: v296.** APK: build 18 / v2.7.
...
- **v296 (2026-09-05) — the glitch/smoothness/code review, 24 fixes + the rest bar + 14 controls.** Invariants it adds:
  ...
  - **Not done, by decision:** the 74 `backdrop-filter` layers on the exercise grid stay (a device trace, not a deduction, would justify removing them); `navigate()` still tears the in-app rest timer down on any screen change (the OS alarm survives).
