...
ledger: 211 confirmed findings, batch → version table, the 65 lows still open,
the owner's actions). Every code change shipped with a check that FAILED first;
every pure refactor shows 0 differences in the fingerprint net. Six new
contracts (66–71; the gate ticks 67).

- **The kg↔lb rule lives once** — `toDisplayWeight` / `toStoredKg` / `kgToUnit`
  in js/ui.js. app.js carried eight per-screen converters and `KG_TO_LB` in
  eleven places, and a probe found 20 disagreements between them (the preference
  converters rounded to 4 decimals where the four copies rounded to 0.01 — they
  now round to 0.01, as the copies always did). **Contract 66** refuses a
  converter written by hand.
- **One context key per concept**: every dated screen reads `ctx.date`, a
  screen with its own unit `ctx.unit`; `dayDate`, `sdDate`, `runDate`, `foodLog`,
  `sdUnit`, `runUnit` are gone from js/ and from the two tools that drive
  screens. **Contract 67** refuses the old spellings AND every key a `navigate()`
  passes must be read by that screen (it caught the review's old `{dow}` shape).
- Dead `groupMusclesFromExercises` + `MUSCLE_INFO` removed (**contract 68**
  refuses an uncalled top-level function); five stale comments rewritten; the
  body.js header lists its real inbound edges; the four auth sheets share one
  error/busy/captcha kit (a 40-state real-Chrome probe is byte-identical before
  and after, in both languages).
- **The identity-layer banner had swallowed a rule since v364:** a `/*` opened
  inside the banner comment turned its prose into the selector of
  `body { --card-border: 0 }`, so body read `1px solid rgba(58,38,22,.10)` —
  nothing rendered differently only because every consumer is zeroed by name.
  Fixed; **contract 69** refuses a comment opened inside a comment.
- Arabic: the streak chip says «يوم واحد» / «يومان» (words, no digit) and
  digits from 3 (`streakFigure`); the seven food presets say «أرز»
  (`test-i18n.js` now reads the 598 catalogue strings too); the cardio icon
  chips are named in the reader's language; the «new cardio type» sub-sheet is
  the SIXTH sheet under `holdSheetFocus` (contract 64 counts 6).
- **Contract 70**: `version.json`'s `apk.url` must be on a host the two openers
  allow (the old Drive instruction would have left the download button dead).
  **Contract 71**: any document that writes out the script order must match
  `scripts/shipped.js`.
- **CLAUDE.md is a guide again** (154 KB, was 514 KB loaded into every
  session): 16 guide sections stay; the 84 release sections since v309 moved
  VERBATIM into this file (byte assertion: 136,108 + 378,973 = the original +
  the pointer + this header). Load-bearing invariants that lived only in a moved
  section got one-line pointers naming the version; corrected along the way:
  «FIVE places rebuild the plan» → SEVEN, «Pending SQL: NONE» → pending/30, the
  three Drive lines, the seven-script order, «No automated test framework», the
  stale counts, and a signing paragraph (`~/.android/debug.keystore` is
  per-machine; the shipped signer's fingerprint; keep an encrypted copy
  off-machine). `README.md` rewritten for a new maintainer; `docs/AUTOMATION.md`
  brought to 14 suites and the current tools; `docs/LLD.md` marked historical.
- **Skipped, recorded in the ledger:** `maintain:modules#4` (DB.* has two write
  disciplines) — touching every storage caller is not a small, provable change.

67 contracts · lint · **14 suites, 0 failed, 0 skipped**. `ux-flows.js` identical
to v400 line for line. Fingerprint: 0 differences on every pure refactor; the
intended diffs are the sixth sheet's role/name/tabindex and the «أرز» names.
